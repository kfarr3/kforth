...
int kforth_print_str_to_file(_kforth_context *context, char *string)
{
	int ret;
    
	platform_write_save_file("%s",string);
...
void kforth_save_word(_kforth_context *context, _word_entry_const *entry)
{
	_word_list_item *word_list;
	...
	{
		kforth_save_word(context, (_word_entry_const*)entry->next);

		platform_write_save_file(": %s ", entry->name);
        ...
        for(word_list=(_word_list_item*)entry->word_list; word_list->type!=END_TYPE; word_list++)
        {
            ...
            {
                case LITERAL_TYPE:
                    platform_write_save_file("%d ", (int)word_list->data);
                    break;
                case WORD_TYPE:
                    platform_write_save_file("%s ", get_name_by_word(context, word_list->data));
                    break;
				case STR_TYPE:
					platform_write_save_file("\"");
					word_list+=kforth_print_str_to_file(context, (char*)(word_list+1));
					platform_write_save_file("\" ");
					break;
				case LABEL_TYPE:
					break;
                ...
                case RECURSIVE_TYPE:
                    ...
                    break;
                default:
                    platform_write_save_file("UNKNOWN TYPE %X ", word_list->type);
                    break;
            }
        }
		platform_write_save_file(";\r\n");
...
void kforth_save(_kforth_context *context, char *file_name)
{
	if (platform_open_save_file(file_name)==0)
	{
		kforth_save_word(context, (_word_entry_const*)context->dictionary);
		platform_close_save_file();
		platform_printf("file saved\r\n");
	}
	else
	{
		platform_printf("Error opening file for saving\r\n");
	}
}
...
void kforth_load(_kforth_context *context, char *file_name)
{
	char *statement;

	if (platform_open_load_file(file_name)==0)
	{
		while((exit_flag==0)&&((statement=platform_read_load_file_statement()))!=NULL)
		{
			kforth_execute_statement(context, statement);
		}
		platform_printf("file loaded!\r\n");
		platform_close_load_file();
	}
	else
	{
		platform_printf("file failed to load!\r\n");
	}
}
...
/*    platform_printf("PARSING\r\n");
	...
	else if (strncmp(statement, "save ", 5)==0)
	{
		kforth_save(context, statement+5);
	}
	else if (strncmp(statement, "load ", 5)==0)
	{
		kforth_load(context, statement+5);
		return exit_flag;
	}
